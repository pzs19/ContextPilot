...
vllm_version = None
VLLM_SLEEP_LEVEL = 1
is_unreleased_dev_build = bool(package_version and ".dev" in package_version and "+g" in package_version)
...
elif vs.parse(package_version) >= vs.parse("0.7.0") or is_unreleased_dev_build:
    vllm_version = package_version
    if vs.parse(package_version) >= vs.parse("0.8.5") or is_unreleased_dev_build:
        VLLM_SLEEP_LEVEL = 2
    from vllm import LLM
    from vllm.distributed import parallel_state
else:
