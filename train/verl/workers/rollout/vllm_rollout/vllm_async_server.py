...
try:
    ...
    try:
        from vllm.utils import get_distributed_init_method as get_tcp_uri
    except ImportError:
...
class vLLMHttpServer:
    ...
    async def launch_server(self, master_address: str = None, master_port: int = None):
        ...
        override_generation_config = dict(
            ...
            max_new_tokens=self.config.multi_turn.single_turn_max_tokens,
        )
        logger.info(f"override_generation_config: {override_generation_config}")
        base_seed = int(os.getenv("CONTEXTPILOT_ROLLOUT_SEED", "0"))
        engine_seed = (base_seed + self.replica_rank) & 0x7FFFFFFF
        logger.info(
            "vLLM replica %s uses engine seed %s (base seed %s)",
            self.replica_rank,
            engine_seed,
            base_seed,
        )
    ...
    async def run_server(self, args: argparse.Namespace):
        ...
        engine_client_kwargs = {
            "vllm_config": vllm_config,
            "usage_context": usage_context,
            "disable_log_stats": engine_args.disable_log_stats,
        }
        if hasattr(engine_args, "disable_log_requests"):
            engine_client_kwargs["disable_log_requests"] = engine_args.disable_log_requests
        else:
            engine_client_kwargs["enable_log_requests"] = engine_args.enable_log_requests
        engine_client = AsyncLLM.from_vllm_config(**engine_client_kwargs)

        reset_mm_cache = getattr(engine_client, "reset_mm_cache", None)
        if callable(reset_mm_cache):
            await reset_mm_cache()
        else:
            logger.info("AsyncLLM.reset_mm_cache is unavailable in this vLLM build; skipping cache reset.")
    ...
    async def run_headless(self, args: argparse.Namespace):
        if CoreEngineProcManager is None:
            raise ImportError("Current vLLM build does not provide CoreEngineProcManager for headless mode")
    ...
    async def generate(
        ...
        model_remaining_tokens = self.config.max_model_len - len(prompt_ids)
        requested_max_tokens = sampling_params.pop("max_tokens", None)
        if requested_max_tokens is None:
            max_tokens = model_remaining_tokens
        else:
            max_tokens = min(model_remaining_tokens, max(1, int(requested_max_tokens)))
        ...
        elif requested_logprobs is None:
            sampling_params["logprobs"] = None
        else:
            sampling_params["logprobs"] = max(0, int(requested_logprobs))
        ...
        if sampling_params.logprobs is not None:
            output_logprobs = final_res.outputs[0].logprobs
            log_probs = [position[token_ids[i]].logprob for i, position in enumerate(output_logprobs)]
            if sampling_params.logprobs > 0:
                vocab_size = int(getattr(self.model_config.hf_config, "vocab_size", 0) or 0)
                entropy_norm = math.log(max(2, vocab_size))
                token_entropies = []
                for position in output_logprobs:
                    top_logprobs = [float(item.logprob) for item in position.values()]
                    truncated_entropy = -sum(math.exp(logp) * logp for logp in top_logprobs)
                    token_entropies.append(float(truncated_entropy / entropy_norm))
        return TokenOutput(token_ids=token_ids, log_probs=log_probs, token_entropies=token_entropies)
    ...
    async def wake_up(self):
        if self.rollout_mode == RolloutMode.HYBRID:
            ...
            if self.node_rank == 0:
                ...
                if callable(wake_up):
                    await wake_up(tags=["kv_cache", "weights"])
                else:
                    logger.info("AsyncLLM.wake_up is unavailable in this vLLM build; skipping wake_up.")
        ...
        if self.rollout_mode == RolloutMode.HYBRID:
            if self.node_rank == 0:
                ...
                if callable(reset_prefix_cache):
                    ...
                    logger.info(
                        "AsyncLLM.reset_prefix_cache is unavailable in this vLLM build; skipping cache reset."
                    )
            ...
            if self.node_rank == 0:
                ...
                if callable(sleep):
                    ...
                    logger.info("AsyncLLM.sleep is unavailable in this vLLM build; skipping sleep.")
        elif self.rollout_mode == RolloutMode.STANDALONE:
    ...
    async def wait_for_requests_to_drain(self):
        ...
        if callable(wait_for_requests_to_drain):
            ...
            logger.info(
                "AsyncLLM.wait_for_requests_to_drain is unavailable in this vLLM build; skipping drain wait."
            )
