...
class HermesToolParser(ToolParser):
    ...
    def _strip_trailing_transport_tokens(self, text: str) -> str:
        cleaned = text.strip()
        suffixes = {
            token
            for token in (
    ...
    async def extract_tool_calls(self, responses_ids: list[int]) -> tuple[str, list[FunctionCall]]:
        ...
        for match in matches:
            try:
                ...
                logger.debug(f"Parsed tool call JSON: {function_call}")
                name = function_call.get("name")
                arguments = function_call.get("arguments", {})
                if name is None:
                    logger.warning(f"Tool call missing 'name' field: {match[:200]}")
                    continue
                function_calls.append(FunctionCall(name=name, arguments=json.dumps(arguments, ensure_ascii=False)))
            except json.JSONDecodeError as e:
                logger.error(f"Failed to parse tool call JSON: {e}. Raw match: {match[:200]}")
            except Exception as e:
                logger.error(f"Failed to decode tool call: {e}. Parsed: {function_call if 'function_call' in dir() else 'N/A'}")

        content = self._strip_trailing_transport_tokens(self.tool_call_regex.sub("", text))
