...
class MultiTurnSFTDataset(Dataset):
    ...
    def _read_files_and_process(self):
        ...
        def parse_messages_if_json_string(messages):
            messages = series_to_item(messages)
        ...
        self.messages = (
            ...
            .apply(parse_messages_if_json_string)
        ...
        if self.enable_thinking_key in self.dataframe.columns:
            self.enable_thinking = (
                self.dataframe[self.enable_thinking_key]
                .apply(series_to_item)
                .apply(lambda v: None if v is None or (isinstance(v, float) and np.isnan(v)) else bool(v))
                .tolist()
            )
        else:
